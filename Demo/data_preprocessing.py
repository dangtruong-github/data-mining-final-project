...
def process_data(text):
    text = delete_copyright(text)
    text = keep_only_latin_and_vietnamese_description(text)
    text = segment_words(text)

    return text

def delete_copyright(text):
    copyright = "Bảo Lưu Mọi Quyền"
    if text[-len(copyright):] == copyright:
        text = text[:-len(copyright)]
    return text

# Cleaning function
def keep_only_latin_and_vietnamese_description(text):
...
def segment_words(text):
    ...
    output = rdrsegment.segmentRawSentences(tokenizer, text)
